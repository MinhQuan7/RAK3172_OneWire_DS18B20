...
void loop() {
  ...
  //___________________________Config RS485_________________________--
  if (getResultMsg(&node, result)) {
  Serial.println();
  double res_dbl = node.getResponseBuffer(0) / 10;
  String res = "Temperature RS485: " + String(res_dbl) + " C\r\n";
  ...
  // Chuyển đổi nhiệt độ thành số nguyên 16-bit (2 byte)
  int16_t temp_int = (int16_t)(temperature * 100); // nhân 100 để giữ lại 2 số sau dấu thập phân
  data[0] = (temp_int >> 8) & 0xFF; // byte cao
  data[1] = temp_int & 0xFF; // byte thấp
  Serial.print("Data 0: ");
...
//_______________________________________________ModBus Rs485______________________________________
